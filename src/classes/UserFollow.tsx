...
export class UserFollow {
    ...
    static async isUserFollowed(userId: string) {
        try {
            ...
            if (token !== null) {
                ...
                const response = await axios.post(`https://217.160.238.71:3333/follow/isUserFollowed`, {
    ...
    static async follow(userId: string) {
        try {
            ...
            if (token !== null) {
                let payload = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString());
                const response = await axios.post(`https://217.160.238.71:3333/follow/follow`, {
    ...
    static async unfollow(userId: string) {
        try {
            ...
            if (token !== null) {
                ...
                const response = await axios.post('https://217.160.238.71:3333/follow/unfollow', {
                        followedUserId: parseInt(userId),
                        userId: parseInt(payload.userId)
                    },
                    {
