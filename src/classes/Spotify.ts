...
class Spotify {
    static async getToken() {
        ...
        try{
            const response = await axios.get(`https://217.160.238.71:3333/spotify/getSpotifyAccess/${payload.userId}`);
    ...
    static async createAxiosInstance() {
        ...
        instance.interceptors.response.use(response => response, async function axiosRetryInterceptor(error) {
            ...
            if (error.response && error.response.status === 401) {
                ...
                const res = await axios.post(`https://217.160.238.71:3333/spotify/refreshToken/${payload.userId}`, {
