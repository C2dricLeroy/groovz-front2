        ...
        try {
            ...
            if (token !== null) {
                try {
                    ...
                    const response = await axios.post(`https://217.160.238.71:3333/post/share`, {
    ...
    static async getPosts() {
        ...
        try {
            ...
            if (token !== null) {
                try {
                    let payload = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString());
                    const response = await axios.get(`https://217.160.238.71:3333/post/${payload.userId}?limit=${limit}&skip=${skip}`)
                    return response.data;
                } catch (error: any) {
                    throw new Error(`Failed to share the Playlist : ${error.message}`)
                }
    ...
    static async getOtherPosts(skip: number) {
        try {
            ...
            if (token !== null) {
                try {
                    let payload = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString());
                    const response = await axios.get(`https://217.160.238.71:3333/post/other/${payload.userId}?skip=${skip}`)
                    return response.data;
                } catch (error: any) {
                    throw new Error(`Failed to share the Playlist : ${error.message}`)
                }
